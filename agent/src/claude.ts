...
export class ClaudeClient {
  ...
  private model: string = 'claude-3-5-sonnet-20241022'; // Default Sonnet model
    ...
    try {
      const response = await this.anthropic.messages.create({
        ...
        messages: [
          {
            role: 'user',
            content: `Here is the user's on-chain financial context as JSON:\n${contextJson}\n\nPlease generate a personalized 1-2 sentence coaching nudge.`
          }
        ],
        temperature: 0.7
      });

      // Safely fetch content block
